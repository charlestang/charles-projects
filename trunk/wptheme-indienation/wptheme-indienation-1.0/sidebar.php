...
?>
	<div id="sidebar" role="complementary">
		...
		<ul role="navigation">
			<?php wp_list_pages('title_li=<h2><span>' . __('Pages') . '</span></h2>' ); ?>

			<li><h2><span><?php _e('Archives'); ?></span></h2>
				<ul>
				<?php wp_get_archives('type=monthly'); ?>
				</ul>
			</li>

			<?php wp_list_categories('show_count=1&title_li=<h2><span>' . __('Categories') . '</span></h2>'); ?>
		</ul>
		<ul>
			<?php /* If this is the frontpage */ if ( is_home() || is_page() ) { ?>
				<?php wp_list_bookmarks(); ?>

				<li><h2><span><?php _e('Meta'); ?></span></h2>
				<ul>
					<?php wp_register(); ?>
